...
#         return torch.cat(outputs, dim=0)
    ...
    def score_with_state(self, seq):
        ...
        rsa = {key: value[None, ...].to(device) for key, value in make_rsa(seq, 3, item_num).items()}
        state = self.calc_hidden_state(**rsa).detach()[:, -1]
        score = self.head(state)

        return score.squeeze(0), state.squeeze(0)
