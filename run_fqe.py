...
def main(_):
    ...
    fh = logging.FileHandler('logs/run_fqe1.log')
    ...
    # training_temp, data_description_temp, testset_valid_temp_cut, testset, holdout_valid_temp_cut, _ = get_dataset(path=filtered_data, splitting='temporal_full', q=0.8)
    ...
    model_e, _, _, _ = globals()[args.config_e.gen_model](args.config_e.params, training_temp, data_description_temp, device, item_embs)
    model_e.load_state_dict(torch.load(args.config_e.chkpt_path, map_location=torch.device(device)))
    model_e.eval()
    
    model_e.num_embeddings = model_e.pos_emb.num_embeddings
    ...
    logger.info("go to extract_state_actions")
    ...
    # np.save("./actions_neg_more.npy", actions_neg)
    actions_neg = actions_neg[:, :1]
    # actions_neg = np.load("./actions_neg_more.npy")[:, :1]
